...
feature_importance_multi = pd.DataFrame(
    {"feature": X_test_var_multi.columns, "importance": rfc_multi.feature_importances_}
...
sns.barplot(
    data=feature_importance_multi.head(10), x="importance", y="feature", orient="h"
)
...
test_prob_binary = rfc_binary.predict_proba(X_test_var_binary)[
    :, 1
]  # for posiive variable MSI
fpr_binary, tpr_binary, thresholds = roc_curve(
    test_y_binary, test_prob_binary, pos_label="MSI"
)
...
plt.figure(figsize=(12, 8))
sns.barplot(
    data=feature_importance_binary.head(10), x="importance", y="feature", orient="h"
)
